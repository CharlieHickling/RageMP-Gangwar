...
var Player = class {
    ...
    set money(val) {
        this._money = val;
        this._player.call("HUD:UpdateMoney", [val]);
    }
    get id() {
        return this._userId
    }
    ...
    getLevelData() {
        ...
        for (var level = 1; level <= 10000; level++) {
            let exp_level = Math.round(base + (faktor * (Math.pow(level, hoch)) / Regular), 2)
            if (level_base <= self._exp) {
                current_level = level;
                current_level_exp = level_base;
            }
            if (level == current_level + 1) {
                next_lvl_exp = level_base;
            }
            level_base += exp_level
        }
        let progress = 100 / (next_lvl_exp - current_level_exp) * (self._exp - current_level_exp)
        return {
            progress: progress,
            level: current_level,
            level_base: current_level_exp,
            exp: self._exp,
            nexp: next_lvl_exp
        }
    }
    exp(mul) {
        let self = this;
        self.log("EXP", self._exp)
        if ((self._exp + (1 * mul)) < 1) return;
        let current_data = self.getLevelData();
        let old_level = self.getLevelData().level;
        self._exp += 1 * mul;
        let new_level = self.getLevelData().level;
        if (new_level > old_level) {
            self._player.setVariable("level", self.getLevelData().level);
            self._player.call("Player:LevelUp", [old_level, new_level]);
            let weapons = Weapons.getWeapons(new_level)
            let unlocks = weapons.level_unlocks;
            let standart = weapons.level_standart;
            if (unlocks != undefined) {
                unlocks = unlocks.map(function(v) {
                    return Weapons.getName(v).fullname
                })
            }
            if (standart != undefined) {
                standart = Weapons.getName(standart).fullname;
            }
            if ((standart != undefined) || (unlocks != undefined)) {
                self._player.call("Player:UnlockWeapons", [standart, unlocks]);
            }
        }
    }
    isDead() {
    ...
    killed(victim, weapon, teamkill) {
        let self = this;
        if (teamkill == true) {
            mp.players.broadcast(`${self._player.name} killed a teammate`);
            self.killStreak("clear");
            self.exp(10);
        } else {
            mp.players.broadcast(`${self._player.name} killed some1`);
            self.killStreak("add");
            let addExp = 10 + (1 * self.killCombo)
            self.exp(addExp);
            self._player.call("Notifications:New", [{
                title: "Kill",
                titleSize: "16px",
